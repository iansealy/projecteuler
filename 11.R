...
rows <- length(strsplit(GRID, "\n")[[1]])
cols <- length(strsplit(strsplit(GRID, "\n")[[1]][1], " ")[[1]])

grid <- GRID
grid <- gsub("\n", " ", grid, perl=TRUE)
grid <- as.integer(strsplit(grid, " ")[[1]])
grid <- matrix(grid, nrow=rows, ncol=cols, byrow=TRUE)
